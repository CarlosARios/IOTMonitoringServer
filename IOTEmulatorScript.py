...
'''
MQTT_HOST = "54.152.234.41"  # "ip.maquina.mqtt"
MQTT_PORT = 8082
...
MQTT_PASSWORD = "Bogota1986"  # "ContraseñaMQTT"
...
'''
Valor medio de la presión en hPa
que el emulador genera y la variación de la presión
'''
PRESSURE_VALUE = 1013.0
PRESSURE_VARIATION = 8.0
...
def process_message(msg: str):
    ...
    if msg.strip().upper().startswith("ALERT"):
        parts = msg.split()
        variable = parts[1] if len(parts) > 1 else "desconocida"
        min_value = parts[2] if len(parts) > 2 else "?"
        max_value = parts[3] if len(parts) > 3 else "?"

        print("############################################################")
        print("############################################################")
        print("############################################################")
        print("              ALERTA: {}                     ".format(msg))
        print("        Variable: {} | Rango: {} - {}".format(
...
def measure_pressure():
    ...
    En emulación, estos datos son aleatorios con distribución uniforme
    desde el valor medio -variación hasta el valor medio +variación.
    Si se utilizara un sensor, acá se debería leer la presión real.
    '''
...
def measure_data():
    ...
    print("\tPresión: {} hPa".format(pressure))
